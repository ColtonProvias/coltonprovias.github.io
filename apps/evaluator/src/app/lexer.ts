...
export class Lexer {
    ...
    private parseIdent(reader: Reader): Token {
        ...
        switch (literal) {
            case 'for': return new Token(TokenType.FOR, 'for');
            case 'if': return new Token(TokenType.IF, 'if');
            case 'else': return new Token(TokenType.ELSE, 'else');
        }
    ...
    private parseNumber(reader: Reader): Token {
        ...
        while (isNum(reader.char) || reader.char === '.') {
            if (hasPeriod && reader.char === '.') throw new Error('invalid number encountered');
            hasPeriod ||= reader.char === '.';
            literal += reader.char;
            reader.next();
            if (reader.isEOF()) break;
        }
        if (!reader.isEOF() && isAlpha(reader.char)) throw new Error('invalid number encountered');
    ...
    private parseOperator(reader: Reader): Token {
        switch (reader.char) {
            ...
            case '(': return new Token(TokenType.LEFT_PARENTHESIS, '(');
            case ')': return new Token(TokenType.RIGHT_PARENTHESIS, ')');
